  ...
  return (
    <div className="space-y-10">
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        {[
          { label: "Active Plans", value: MOCK_SUBSCRIPTIONS.filter((s) => s.status === "active").length, icon: "📋" },
          { label: "Upcoming Deliveries", value: MOCK_DELIVERIES.filter((d) => d.status !== "delivered" && d.status !== "cancelled").length, icon: "🚚" },
          { label: "Items Delivered", value: MOCK_DELIVERIES.filter((d) => d.status === "delivered").flatMap((d) => d.items).length, icon: "✅" },
        ].map((stat) => (
          <Card key={stat.label}>
